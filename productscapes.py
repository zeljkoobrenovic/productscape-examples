...
def main():
    ...
    toolkit = Path(override or config.get('toolkit', '../productscapes')).expanduser()
    ...
    if not entry.is_file() or not (toolkit / '_wiring/project_paths.py').is_file():
        raise ValueError(f'Productscapes toolkit not found at {toolkit}. Clone productscapes beside this repository, '
